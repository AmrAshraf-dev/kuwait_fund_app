...
import 'package:kf_ess_mobile_app/features/insurance/presentation/widgets/can_not_add_insurance_widget.dart';
import 'package:kf_ess_mobile_app/features/insurance/presentation/widgets/create_insurance_request_widget.dart';
import 'package:kf_ess_mobile_app/features/insurance/presentation/widgets/view_only_insurance_widget.dart';
...
class _InsuranceScreenState extends State<InsuranceScreen> {
  ...
  Widget build(BuildContext context) {
    return MasterWidget(
      ...
      appBarHeight: 90.h,
      widget: BlocProvider(
        create: (context) => insuranceCubit..getInsuranceMasterInfo(),
        child: Padding(
          ...
          child: BlocBuilder<InsuranceCubit, InsuranceState>(
            builder: (context, state) {
              ...
              } else if (state is CanAddInsuranceState) {
                return CreateInsuranceRequestWidget(
                    insuranceEntity: state.insuranceEntity);
              } else if (state is ViewOnlyInsuranceState) {
                return ViewOnlyInsuranceWidget(
                    insuranceEntity: state.insuranceEntity,
                    showCancelButtom: state.showCancelButton,
                    insuranceCubit: insuranceCubit);
              } else if (state is CanNotAddInsuranceState) {
                ViewsToolbox.dismissLoading();

                return CanNotAddInsuranceWidget(
                  insuranceEntity: state.insuranceEntity,
                );
              }
              ...
              //             subtitle:
              //                 "${context.tr("insurance_id")} ${insurance?.insuranceCompanyName}",
              //             status: insurance?.insurenceStatus,
              //           ),
              //           Divider(thickness: 1),
              //           InsuranceRowDetails(
